...
class BatchCallbackController extends ControllerBase {
  ...
  public function execute(Request $request): JsonResponse {
    ...
    $data = json_decode($body, true) ?: []; $op = $data['op'] ?? NULL; if (!$op) return new JsonResponse(['error' => 'missing op'], 400);
    try { $res = $this->invoke($op['callable'] ?? [], $op['args'] ?? []); return new JsonResponse(['ok' => true, 'result' => $res]); }
